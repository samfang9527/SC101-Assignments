...
def add_data_for_name(name_data, year, rank, name):
    ...
    if name in name_data:
        name_value = name_data[name]
        if year in name_value:
            year_value = int(name_value[year])
            if int(rank) < year_value:
                name_value[year] = int(rank)
        else:
            name_value[year] = int(rank)
    else:
    ...
    with open(filename, 'r') as f:
        for lines in f:
            ...
            if len(line_list) == 1:
                ...
                rank = line_list[0]
                rank = rank.strip()
                male_name = line_list[1]
                male_name = male_name.strip()
                female_name = line_list[2]
